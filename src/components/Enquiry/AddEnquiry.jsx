import { useState, useEffect } from "react";
    ...
    const AddEnquiry = ({ toggleModel, editUser }) => {
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        console.log("Submitting enquiry with data:", formData); // Log the form data
        try {
          if (editUser) {
            // Update the enquiry if editUser is present
            await axios.put(`http://localhost:9090/enquiry/${editUser._id}`, formData);
            dispatch(updateUser({ ...formData, id: editUser._id }));
          } else {
          
            const response = await axios.post("http://localhost:9090/enquiry/enquiry-form", formData);
            console.log("API Response:", response.data);
            dispatch(addData(response.data));
          }
      
          toggleModel(); // Close the modal after submitting
        } catch (error) {
      ...
      return (
        <>
          <section className="enquiry-form">
            <div className={`max-w-96 sm:max-w-xl md:max-w-4xl mx-auto mt-6 bg-white rounded-md shadow-md duration-1000`}>
                ...
                >
                  ...
                  <div>
                    ...
                    <select
                      id="courseCategory"
                      name="courseCategory"
                      value={formData.courseCategory} onChange={handleChange}
                      className="block w-full h-10 py-2 px-4 mt-2 
                    text-gray-600  border rounded-md focus:border-yellow-400 focus:ring-yellow-300 focus:ring-opacity-40 focus:outline-none focus:ring"
                    >
                      <option defaultValue="Select Category">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="finalizeFees"
                      type="text"
                      value={formData.finalizeFees} onChange={handleChange} 
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.academicQualification} onChange={handleChange} 
                      className="block w-full px-4 py-2 mt-2 text-gray-600 bg-white
                    border border-gray-200 rounded-md focus:border-yellow-400 focus:ring-yellow-300 focus:ring-opacity-20  focus:outline-none focus:ring no-arrows"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.yearOfPassing} onChange={handleChange} 
                      className="block w-full px-4 py-2 mt-2 text-gray-600 bg-white
                    border border-gray-200 rounded-md focus:border-yellow-400 focus:ring-yellow-300 focus:ring-opacity-20  focus:outline-none focus:ring no-arrows"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.sourceOfEnquiry} onChange={handleChange} 
                      className="block w-full px-4 py-2 mt-2 text-gray-600 bg-white border border-gray-200 rounded-md focus:border-yellow-400 focus:ring-yellow-300
                  focus:ring-opacity-20  focus:outline-none focus:ring"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      name="referralBy"
                      type="text"
                      value={formData.referralBy} onChange={handleChange} 
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      ...
                      name="followUp"
                      type="text"
                      value={formData.followUp} onChange={handleChange} 
